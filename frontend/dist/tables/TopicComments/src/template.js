...
const createAuthorTag = (value, styles) => {
    ...
    if (words[0] && words[0][0]) {
        ...
        if (words[0][1])
            secondLetter = words[0][1].toUpperCase();
    }
...
export const topicCommentsTemplate = (state) => {
    const { topic, styles, viewpoint, actions: _actions } = state;
    const actions = { delete: true, ..._actions };
    const onTableCreated = (e) => {
        if (!e)
            return;
        const table = e;
      ...
      `;
        ...
        let comments = topic.comments.values();
        if (viewpoint) {
            comments = [...topic.comments.values()].filter((comment) => comment.viewpoint === viewpoint);
        }
    ...
    return BUI.html `<bim-table @cellcreated=${({ detail }) => {
        const { cell } = detail;
