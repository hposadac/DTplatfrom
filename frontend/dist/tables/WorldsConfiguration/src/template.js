...
const getWorldGrid = (components, world) => {
    const grids = components.get(OBC.Grids);
    const grid = [...grids.list.values()].find((grid) => grid.world === world);
    return grid;
};
const createLightColorInput = (light, defaultValue) => {
...
const createAOColorInput = (renderer, defaultValue) => {
...
const createAOEnabledInput = (renderer, defaultValue) => {
    const { postproduction } = renderer;
    const onInputChange = (e) => {
        const input = e.target;
...
const createGammaEnabledInput = (renderer, defaultValue) => {
    const { postproduction } = renderer;
    const onInputChange = (e) => {
        const input = e.target;
...
const createCEEnabledInput = (renderer, defaultValue) => {
    const { postproduction } = renderer;
    const onInputChange = (e) => {
        const input = e.target;
...
const createCheckboxInput = (obj, key, defaultValue, onInputSet = () => { }) => {
    const onInputChange = (e) => {
        const input = e.target;
...
const createNumberInput = (obj, key, defaultValue, _config) => {
    const config = {
        slider: false,
        min: 0,
        max: 100,
 ...
 * Heloooooooooo
 */
export const worldsConfigurationTemplate = (state) => {
    ...
    const onCellCreated = ({ detail, }) => {
        ...
        if (!parent)
            return;
        const nameCell = parent.querySelector("bim-table-cell[column='Name']");
        const valueCell = parent.querySelector("bim-table-cell[column='Value']");
        if (nameCell && !valueCell) {
            nameCell.style.gridColumn = "1 / -1";
        }
    };
    const onTableCreated = async (element) => {
        if (!element)
            return;
        const table = element;
        ...
        table.dataTransform = {
            Value: (value, data) => {
                ...
                if (configName === "Distance" &&
                    ...
                    if (!worldGrid)
                        return value;
                    return createNumberInput(worldGrid.material.uniforms.uDistance, "value", value, { slider: true, min: 300, max: 1000 });
                }
                if (configName === "Size" &&
                    data.IsGridConfig &&
                    typeof value === "string") {
                    ...
                    const xInput = createNumberInput(worldGrid.material.uniforms.uSize1, "value", x, { slider: true, suffix: "m", prefix: "A", min: 1, max: 20 });
                    const yInput = createNumberInput(worldGrid.material.uniforms.uSize2, "value", y, { slider: true, suffix: "m", prefix: "B", min: 1, max: 20 });
                    return BUI.html `
            <div style="display: flex; gap: 0.25rem; width: 100%; flex-wrap: wrap">${xInput}${yInput}</div>
          `;
                }
                if (configName === "Near Frustum" &&
                ...
                if (configName === "Invert Drag" &&
                    camera.hasCameraControls() &&
                    typeof value === "boolean") {
                    return createCheckboxInput(camera.controls, "dollyDragInverted", value);
                }
                if (configName === "Dolly Speed" &&
                    camera.hasCameraControls() &&
                    typeof value === "number") {
                ...
                if (configName === "Intensity" && typeof value === "number") {
                    if (data.Light && typeof data.Light === "string") {
                        const light = scene.three.children.find((child) => child.uuid === data.Light);
                    ...
                    if (data.IsAOConfig &&
                        ...
                        return createNumberInput(renderer.postproduction.n8ao.configuration, "intensity", value, { slider: true, max: 16, step: 0.1 });
                    }
                }
                if (configName === "Color" && typeof value === "string") {
                    const lightID = data.Light;
                    const light = scene.three.children.find((child) => child.uuid === lightID);
                    if (light && light instanceof THREE.Light) {
                ...
                if (configName === "Half Resolution" &&
                    data.IsAOConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "boolean") {
                    return createCheckboxInput(renderer.postproduction.n8ao.configuration, "halfRes", value);
                }
                if (configName === "Screen Space Radius" &&
                    data.IsAOConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "boolean") {
                    return createCheckboxInput(renderer.postproduction.n8ao.configuration, "screenSpaceRadius", value);
                }
                if (configName === "Radius" &&
                    data.IsAOConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.n8ao.configuration, "aoRadius", value, { slider: true, max: 2, step: 0.1 });
                }
                if (configName === "Denoise Samples" &&
                    data.IsAOConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.n8ao.configuration, "denoiseSamples", value, { slider: true, min: 1, max: 16 });
                }
                if (configName === "Samples" &&
                    data.IsAOConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.n8ao.configuration, "aoSamples", value, { slider: true, min: 1, max: 16 });
                }
                if (configName === "Denoise Radius" &&
                    data.IsAOConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.n8ao.configuration, "denoiseRadius", value, { slider: true, min: 0, max: 16, step: 0.1 });
                }
                if (configName === "Distance Falloff" &&
                    data.IsAOConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.n8ao.configuration, "distanceFalloff", value, { slider: true, min: 0, max: 4, step: 0.1 });
                }
                if (configName === "Directional Light" &&
                    data.Light &&
          ...
          `;
                ...
                if (configName === "Tolerance" &&
                    data.IsOutlineConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.customEffects, "tolerance", value, { slider: true, min: 0, max: 6, step: 0.01 });
                }
                if (configName === "Outline" &&
                    data.IsOutlineConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "boolean") {
                    return createCheckboxInput(renderer.postproduction.customEffects, "outlineEnabled", value);
                }
                if (configName === "Gloss" &&
                    data.IsGlossConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "boolean") {
                    return createCheckboxInput(renderer.postproduction.customEffects, "glossEnabled", value);
                }
                if (configName === "Min" &&
                    data.IsGlossConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.customEffects, "minGloss", value, { slider: true, min: -0.5, max: 0.5, step: 0.01 });
                }
                if (configName === "Max" &&
                    data.IsGlossConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.customEffects, "maxGloss", value, { slider: true, min: -0.5, max: 0.5, step: 0.01 });
                }
                if (configName === "Exponent" &&
                    data.IsGlossConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "number") {
                    return createNumberInput(renderer.postproduction.customEffects, "glossExponent", value, { slider: true, min: 0, max: 5, step: 0.01 });
                }
                if (configName === "Gamma Correction" &&
                    data.IsGammaConfig &&
                    renderer instanceof OBF.PostproductionRenderer &&
                    typeof value === "boolean") {
        ...
        for (const [, world] of worlds.list) {
            ...
            const row = {
                data: {
                    Name: world instanceof OBC.SimpleWorld
            ...
            if (scene) {
                ...
                if (worldGrid) {
                    ...
                    if (!sceneRow.children)
                        sceneRow.children = [];
                    sceneRow.children.push(gridRow);
                }
                const directionalLights = scene.three.children.filter((child) => child instanceof THREE.DirectionalLight);
                ...
                const ambientLights = scene.three.children.filter((child) => child instanceof THREE.AmbientLight);
                                    ...
                                    children: [
                                        {
                                            data: {
                                                Name: "Color",
                                                get Value() {
                                                    const color = new THREE.Color(postproduction.customEffects.lineColor);
